...
var STAT = {
	...
	set_isonline: function(){
		$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'is_online','data':{girl:$('#user-info p:eq(1)').text(),site:'habibidate_chat'}},function(){});
	},
	set_storage_count: function(id){
		if(SWMID.var_status=='start'||SWMID.var_status=='pause'){
			$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'set_storage_count','data':{girl:$('#user-info p:eq(1)').text(),storage_id:STAT.var_storage_id,json:(id!=null?{id:id,count:STAT.var_count_send}:{count:STAT.var_count_send}),site:'habibidate_chat'}},function(d){
				if(d.data!=null){
	...
	rem_storage_list:function(list,type){
		if(type&&list){
			$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'rem_storage_list','data':{girl:$('#user-info p:eq(1)').text(),json:{list:list,type:type},site:'habibidate_chat'}},function(d){
				console.log(d);
			});
		}
	},
	sink_storage_list:function(list,type,call){
		if(type){
			$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'sink_storage_list','data':{girl:$('#user-info p:eq(1)').text(),json:{list:list,type:type},site:'habibidate_chat'}},function(d){
				if(call){call(d);}
			});
		}
	},
	set_complete: function(){
		...
		var actualCode = '(' + function() {
			$(document).ajaxComplete(function( event, xhr, settings ) { 
				if(settings.url.indexOf('http://chat.habibidate.com/updates/')!=-1){
				...
				if(settings.url.indexOf('http://chat.habibidate.com/send-message/')!=-1){
					if(settings.url.indexOf('?chat=true')==-1){
						var id_man = settings.url.split('http://chat.habibidate.com/send-message/').join('');
						var lsh = $('#smiles').text();
	...
	get_toserver:function(){
		...
		if(attentions){
			$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'is_attentions','data':{girl:$('#user-info p:eq(1)').text(),json:attentions,site:'habibidate_chat'}},function(){});
		}
		if(status){
			STAT.is_chats(status);
			$.post('https://wmidbot.com/ajax.php',{'module':'statistics','event':'is_status','data':{girl:$('#user-info p:eq(1)').text(),json:status,site:'habibidate_chat'}},function(){});
		}		
		if(unreads){
			console.log(unreads);
		...
		$.each(json,function(i,json){
			...
			if(json.updates[0].girl.chats.length>0){
				$.each(json.updates[0].girl.chats,function(i,t){
					...
					if(_el_li.length==0){
						$.getJSON('http://chat.habibidate.com/chat/updates/member/'+t['client-id']+'/?member-with='+t['client-id'],function(r){ SWMID.set_mansList(r); });
						if(arr_history_smile.join().search(t['client-id'])==-1){
							...
							setTimeout(function(){ 
								...
								if(arr_history_smile.join().search(t['client-id'])==-1){
									...
									$.post("http://chat.habibidate.com/send-message/"+t['client-id'],{tag:t['client-id'],source:'lc',message:msg},function(s){ console.log('post auto smile'); });
								}
							},30000);
						}
					}
...
var SWMID = {
	...
	send:function(request){
		...
		if(limit_send>-1){
			...
			if(SWMID.var_age_from<=SWMID.var_age_to){
				...
				$.post("http://chat.habibidate.com/send-message/"+send_member['id']+"?chat=true",{tag:send_member['id'],source:'lc',message:message},function(d){ })
				.fail(function(s) {
					...
					$.post("http://chat.habibidate.com/send-message/"+send_member['id']+"?chat=true",{tag:send_member['id'],source:'lc',message:''},function(d){ });
				}
				$('#sending_list').prepend('<li onclick="javascript:window.location.href=\'http://chat.habibidate.com/chat/#/'+send_member['id']+'\'">'+send_member.name+' (ID:'+send_member['public-id']+')</li>');
	...
	set_mansList:function(r){
		if(r!=null){ r=r[0];
			...
			if($('#chat_act li#m_'+id).size()==0){
				$('#chat_act ul').append('<li class="cl '+(location.hash=="#/"+id?'active':'blink')+'" onclick="window.location.href=\'http://chat.habibidate.com/chat/#/'+id+'\'" id="m_'+id+'" rel = "'+id+'"><span class="ics chat"></span> '+name+' (ID:'+public_id+')</li>');
	...
	ajax_complete:function(){
		...
		$( document ).ajaxComplete(function( event, xhr, settings ) {
			if(settings.url.indexOf('http://chat.habibidate.com/send-message/')!=-1&&SWMID.var_status == 'start'){
...
function set_mans(req,c){
			var public_name = req[0].updates[0].member.name,
				...
				if($('#chat_act ul li#m_'+id).size()==0){
					$('#chat_act ul').append('<li class="cl '+active+'" onclick="window.location.href=\'http://chat.habibidate.com/chat/#/'+id+'\'" id="m_'+id+'" rel = "'+id+'"><span class="ics '+status+'"></span> '+public_name+' (ID:'+public_id+')</li>');
				}
				
				$('.chat_act li').click(function(){
					window.location.href="http://chat.habibidate.com/chat/#/"+$(this).attr('rel');
				});
